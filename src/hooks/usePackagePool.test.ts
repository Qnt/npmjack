...
describe('createPackagePoolRetryPolicy', () => {
  it('stops retrying non-retryable API errors', () => {
    ...
    expect(
      retry(1, new PackagePoolError('bad request', 'INVALID_TYPE', false))
    ).toBe(false)
  })
  ...
  it('retries retryable API errors up to the cap', () => {
    ...
    expect(
      retry(1, new PackagePoolError('timed out', 'UPSTREAM_TIMEOUT', true))
    ).toBe(true)
    expect(
      retry(3, new PackagePoolError('timed out', 'UPSTREAM_TIMEOUT', true))
    ).toBe(false)
  })
})
